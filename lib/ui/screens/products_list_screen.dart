...
class _ProductsListViewState extends State<_ProductsListView> {
            ...
            child: BlocBuilder<ProductsCubit, ProductsState>(
              builder: (context, state) {
                ...
                return switch (state) {
                  ...
                  ProductsLoaded(products: final products) when products.isEmpty => EmptyView(
                      message: 'No products yet.\nTap + to add your first product!',
                      icon: Icons.inventory_2_outlined,
                      onAction: _navigateToCreateProduct,
                      actionLabel: 'Add Product',
                    ),
                  ProductsLoaded(products: final products, meta: final meta) =>
                    _buildProductsGrid(products, canLoadMore: meta.hasMore),
                  ProductsError(message: final message, previousProducts: final products) when products.isEmpty =>
                    ErrorView(
                      message: message,
                      onRetry: () => context.read<ProductsCubit>().refresh(),
                    ),
                  ProductsError(message: final message, previousProducts: final products) =>
  ...
  Widget _buildStatsBar(ProductsLoaded state) {
  ...
  Widget _buildProductsGridWithError(List<Product> products, String error) {
    return Column(
      children: [
        ...
        // Products grid with previous data
        Expanded(
